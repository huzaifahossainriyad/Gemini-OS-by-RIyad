...
export async function* streamAppContent(
  ...
  const systemPrompt = getSystemPrompt(currentMaxHistoryLength, language); // Generate system prompt dynamically
...
Generate the HTML content for the window's content area only:`;
  ...
  try {
    const response = await ai.models.generateContentStream({
      model: model,
      contents: fullPrompt,
      // Disabling thinking to prioritize speed and reduce latency as per user request.
